...
keyCombo = process.argv[2] + ':' + process.argv[3];
keyComboEncode = encodeURI(keyCombo);
keyComboEncode64 = base64.encode(keyComboEncode);
let bearerToken = '';
let respDict = {};
let tweetDict = {};
request.post({url: 'https://api.twitter.com/oauth2/token',
	      headers: {'Authorization': 'Basic ' + keyComboEncode64,
	      ...
	      body: 'grant_type=client_credentials'
	       ...
	       request({url: 'https://api.twitter.com/1.1/search/tweets.json',
			...
			qs: {'q': process.argv[4]}
			 ...
			 for (let i = 0; i < 5; i++) {
			   status = tweetDict.statuses[i];
			   output = '[' + status.user.id + '] ' + status.text + ' by ' + status.user.name;
			   console.log(output);
			 }
		       });
	     });
